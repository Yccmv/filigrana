...
def applica_effetti(img: Image.Image, p: dict) -> Image.Image:
    ...
    # 9. Schiarimento / whitening (spinge i toni chiari verso il bianco puro)
    #    Simula carta bianca brillante o scanner con luce intensa.
    #    Tecnica: curve di tipo "levels" — schiaccia i neri verso il basso
    #    e alza i bianchi, poi un boost di brightness sulle alte luci.
    if p.get('white_enabled'):
        strength = p.get('white_strength', 0.3)   # 0=nessun effetto, 1=massimo
        arr5 = np.array(img, dtype=np.float32)
        # Livelli: il punto bianco scende da 255 a (255 - strength*80)
        # → i grigi chiari diventano bianchi
        white_pt = max(1.0, 255.0 - strength * 80.0)
        arr5 = np.clip(arr5 / white_pt * 255.0, 0, 255)
        # Gamma boost sulle alte luci (curva convessa → schiarisce i mezzitoni)
        gamma = 1.0 - strength * 0.55   # 1.0=neutro, 0.45=molto chiaro
        gamma = max(0.3, gamma)
        arr5 = 255.0 * np.power(arr5 / 255.0, gamma)
        img  = Image.fromarray(np.clip(arr5, 0, 255).astype(np.uint8), 'RGB')
...
def rasterizza_pagina(pdf_bytes: bytes, dpi: int, max_rot: float,
                       ...
                       fmt: str, q_jpeg: int, colore: str,
                       output_scale: float = 1.0) -> Image.Image:
    """
    ...
    # Riduzione dimensione output (per file più leggeri)
    if output_scale < 0.99:
        new_w = max(1, int(img.width  * output_scale))
        new_h = max(1, int(img.height * output_scale))
...
def _worker(args):
    (pdf_path, wm_bytes, wm_mode, out_dir,
     ...
     output_scale, progress_dict, task_id) = args
...
class PdfSporcaApp(TkinterDnD.Tk):
    ...
    def _build_ui(self):
        ...
        self.white_var        = BooleanVar(value=False)
        self.white_strength   = DoubleVar(value=0.3)
        self.white_max        = DoubleVar(value=1.0)
        ...
        effetti_defs = [
            ...
            (self.white_var,    self.white_strength,  self.white_max,       "Schiarimento ☀",   "Forza"),
        ]
        ...
        ttk.Scale(rcomp, from_=1, to=100, orient=HORIZONTAL,
                  variable=self.jpeg_quality_var).pack(
                  side=LEFT, padx=2, fill=X, expand=True)
        ttk.Entry(rcomp,textvariable=self.jpeg_quality_var,
                  width=3).pack(side=LEFT,padx=1)

        # Riga 2: riduzione dimensione output
        rsize = ttk.Frame(cf); rsize.pack(fill=X, pady=2)
        _lbl(rsize, "Dimensione output:")
        self.output_scale_var = IntVar(value=100)
        sl_size = ttk.Scale(rsize, from_=10, to=100, orient=HORIZONTAL,
                            variable=self.output_scale_var)
        sl_size.pack(side=LEFT, padx=2, fill=X, expand=True)
        ttk.Entry(rsize, textvariable=self.output_scale_var,
                  width=3).pack(side=LEFT, padx=1)
        ttk.Label(rsize, text="%").pack(side=LEFT)
        self._size_hint = StringVar(value="")
        ttk.Label(rsize, textvariable=self._size_hint,
                  foreground='#666', font=('TkDefaultFont',8),
                  width=28).pack(side=LEFT, padx=(8,2))

        # Aggiorna hint peso stimato in tempo reale
        def _upd_size_hint(*_):
            pct = self.output_scale_var.get()
            dpi = self.dpi_var.get() if hasattr(self,'dpi_var') else 150
            # A4 a DPI dato: pixel ≈ (dpi*8.27) × (dpi*11.69)
            px  = int(dpi * 8.27 * pct/100) * int(dpi * 11.69 * pct/100)
            fmt = self.img_format_var.get() if hasattr(self,'img_format_var') else 'JPEG'
            q   = self.jpeg_quality_var.get() if hasattr(self,'jpeg_quality_var') else 85
            if fmt == 'JPEG':
                bpp  = 0.15 + (q / 100) * 1.2   # bit per pixel stimati JPEG
                kb   = px * bpp / 8 / 1024
            else:
                kb   = px * 3 / 1024              # PNG non compresso (stima alta)
            hint = f"≈ {kb:.0f} KB/pag  ({int(dpi*8.27*pct/100)}×{int(dpi*11.69*pct/100)} px)"
            self._size_hint.set(hint)

        self.output_scale_var.trace_add('write', _upd_size_hint)
        self.img_format_var.trace_add('write', _upd_size_hint)
        self.jpeg_quality_var.trace_add('write', _upd_size_hint)
        self.after(200, _upd_size_hint)   # primo aggiornamento dopo build
    ...
    def _traccia_modifiche(self):
        for v in [self.scale_var, self.mode_var, self.opacity_var,
                  ...
                  self.white_var, self.white_strength,
    ...
    def _save_cfg(self):
        try:
            cfg = {k: getattr(self, k).get() for k in [
                ...
                'white_var','white_strength','white_max',
                'img_format_var','jpeg_quality_var','output_scale_var',
                'colore_mode_var','suffisso_var','salva_in_origine_var',
            ]}
    ...
    def _get_effetti(self):
        return {
            ...
            'white_enabled':      self.white_var.get(),
            'white_strength':     self.white_strength.get(),
        }
    ...
    def _anteprima(self, update_only=False):
        ...
        out_scale = self.output_scale_var.get() / 100.0
